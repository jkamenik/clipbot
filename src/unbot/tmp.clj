...
;;   ((monad/lift +) (right 1) (right 4) ;; (left "error happened here")
;;    ))
...
;; ;; Rx.Subscription API
;; (defrecord RxDisposable [messages-ref inner-disposable]
...
;;      (let [sd-attach-disposable (disposable/new-single-assignment-disposable)

;;            on-next          #(rx/on-next observer (String. %))
;;            on-completed     #(do (rx/on-completed observer)
;;                                  (disposable/dispose sd-attach-disposable))
...
;;            _
;;            (disposable/set-disposable sd-attach-disposable
...
;;         observable (.subscribeOn (.observeOn observable scheduler) scheduler)]
